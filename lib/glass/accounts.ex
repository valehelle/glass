defmodule Glass.Accounts do
  ...
  def get_user_by_magic_link(token) do
    {:ok, claims} = Glass.Accounts.Guardian.decode_and_verify(token)
    %{"sub" => sub, "hash_uuid" => hash_uuid} = claims
    user = get_user!(sub)
    {user, hash_uuid}
  end

  def generate_oauth_token(user) do
    uuid = Ecto.UUID.generate()
    hash_uuid = Bcrypt.hash_pwd_salt(uuid)
    custom_claims = %{
      hash_uuid: hash_uuid, 
    }
    {:ok, token, claims} = Glass.Accounts.Guardian.encode_and_sign(user,custom_claims)
    {token, uuid}
  end
  ...
  def get_user_by_session_token(token) do
    ...
    Repo.one(query) |> Repo.preload([:basic, :languages, :skills, projects: [:keywords],
      works: (from w in Work, order_by: [desc: w.start_date]),
      educations: (from e in Education, order_by: [desc: e.start_date]),
      blogs: (from b in Blog, order_by: [asc: b.id], preload: [:tags])
    ])
  ...
  Deletes the signed token with the given context.
  """
  def delete_guardian_token(token) do
    Glass.Accounts.Guardian.revoke(token)
    :ok
  end
