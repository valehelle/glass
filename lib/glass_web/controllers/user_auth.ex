defmodule GlassWeb.UserAuth do
  import Plug.Conn
  import Phoenix.Controller

  alias Glass.Accounts
  alias GlassWeb.Router.Helpers, as: Routes

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in UserToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_glass_web_user_remember_me"
  @uuid_cookie "_uuid"
  ...
  def magic_link_log_in_user(conn, user, hash_uuid) do


    conn = fetch_cookies(conn, signed: [@uuid_cookie])
    uuid = conn.cookies[@uuid_cookie]

    if uuid && Bcrypt.verify_pass(uuid, hash_uuid)  do
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      GlassWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    conn
    |> renew_session()
    |> delete_resp_cookie(@remember_me_cookie)
    |> delete_resp_cookie(@uuid_cookie)
    |> redirect(to: "/")
  end
